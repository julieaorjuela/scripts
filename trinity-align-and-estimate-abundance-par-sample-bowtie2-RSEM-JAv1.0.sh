...
if [ $fastq != "" ] ; then
	...
	do
		if [[ "$f" =~ "R1.fastq" ]] || [[ "$f" =~ "R1.fastq.gz"  ]] || [[ "$f" =~ "R1.fq"  ]] || [[ "$f" =~ "R1.fq.gz"  ]]; then
			...
			echo "# Loading modules"  >> $SHPath"/"$count"_trinity-align-and-estimate-abundance-par-sample.sh"
			#echo "module load toggleDev" >> $SHPath"/"$count"_trinity-align-and-estimate-abundance-par-sample.sh"
			echo "module load system/perl/5.24.0" >> $SHPath"/"$count"_trinity-align-and-estimate-abundance-par-sample.sh"
			echo "module load bioinfo/trinityrnaseq/2.5.1" >> $SHPath"/"$count"_trinity-align-and-estimate-abundance-par-sample.sh" 
			echo "module load bioinfo/bowtie2/2.2.9" >> $SHPath"/"$count"_trinity-align-and-estimate-abundance-par-sample.sh" 
			...
			echo ""perl /usr/local/trinityrnaseq-2.5.1/util/align_and_estimate_abundance.pl --transcripts "$fasta" --seqType fq --left $R1 --right $R2 --est_method RSEM --aln_method bowtie2 --trinity_mode --prep_reference --coordsort_bam --output_dir "$shortName"_bowtie2-rsem_outdir"" >> $SHPath"/"$count"_trinity-align-and-estimate-abundance-par-sample.sh"
			       
			#Printing command executed
			echo "cmd=\" perl /usr/local/trinityrnaseq-2.5.1/util/align_and_estimate_abundance.pl --transcripts "$fasta" --seqType fq --left $R1 --right $R2 --est_method RSEM --aln_method bowtie2 --trinity_mode --prep_reference --coordsort_bam --output_dir "$shortName"_bowtie2-rsem_outdir  \"" >> $SHPath"/"$count"_trinity-align-and-estimate-abundance-par-sample.sh"
