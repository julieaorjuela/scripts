...
if [ $fasta != "" ] ; then
	...
	do
		if [[ "$f" =~ ".fasta" ]] || [[ "$f" =~ ".fasta.gz"  ]] || [[ "$f" =~ ".fa"  ]] || [[ "$f" =~ "R1.fa.gz"  ]]; then
			...
			#banques
			BANK="/data/projects/banks/" #uniref90 is in BANK; uniprot_sprot.* sont dans $BANK/Uniprot
			DB="/usr/local/Trinotate-3.0.1" #Pfam-A.hmm* is in DB
			...
			echo ""scp $BANK/Uniprot/uniprot_spro* \$pathToScratch/DB/"" >> $SHPath"/"$count"_"$softName".sh" # on transfer uniprot_sprot
			...
			echo ""hmmscan --cpu 8 --domtblout pfam_longorfs.domtblout $DBS/Pfam-A.hmm $transDecoderRepertory/longest_orfs.pep "" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\" hmmscan --cpu 8 --domtblout pfam_longorfs.domtblout $DBS/Pfam-A.hmm $transDecoderRepertory/longest_orfs.pep  \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
			#OK diamond
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			...
			#OK blastp diamP_uniprot ## en root index la base uniref90 avec diamond /usr/local/diamond-0.8.29/diamond makedb --db uniref90.fasta --in uniref90.fasta
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			echo "# blastp diamP_uniprott " >> $SHPath"/"$count"_"$softName".sh"
			#--more-sensitive >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<
			echo "" /usr/local/diamond-0.8.29/diamond blastp --query $resultsDir/$shortName.fasta.transdecoder.pep --threads 10 --db $DBS/uniprot_sprot --out $resultsDir/diamP_uniprot.outfmt6 --outfmt 6 --max-target-seqs 1   "" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\" /usr/local/diamond-0.8.29/diamond blastp --query $resultsDir/$shortName.fasta.transdecoder.pep --threads 10 --db $DBS/uniprot_sprot --out $resultsDir/diamP_uniprot.outfmt6 --outfmt 6 --max-target-seqs 1   \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
			
			#OK blastp diamP_uniref90
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			echo "# blastp diamP_uniref90 " >> $SHPath"/"$count"_"$softName".sh"
			echo "" /usr/local/diamond-0.8.29/diamond blastp --query $resultsDir/$shortName.fasta.transdecoder.pep --threads 10  --db $DBS/uniref90.fasta --out $resultsDir/diamP_uniref90.outfmt6 --outfmt 6 --max-target-seqs 1  "" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\" /usr/local/diamond-0.8.29/diamond blastp --query $resultsDir/$shortName.fasta.transdecoder.pep --threads 10 --db $DBS/uniref90.fasta --out $resultsDir/diamP_uniref90.outfmt6 --outfmt 6 --max-target-seqs 1   \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
			
			#OK blastx diamX_uniprot
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			echo "# blastx diamX_uniprot " >> $SHPath"/"$count"_"$softName".sh"
			echo "" /usr/local/diamond-0.8.29/diamond blastx --query $fasta --threads 10 --db $DBS/uniprot_sprot --out diamX_uniprot.outfmt6 --outfmt 6 --max-target-seqs 1   "" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\"  /usr/local/diamond-0.8.29/diamond blastx --query $fasta --threads 10 --db $DBS/uniprot_sprot --out diamX_uniprot.outfmt6 --outfmt 6 --max-target-seqs 1  \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
			
			#OK blastx diamX_uniref90
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			echo "# blastx diamX_uniref90 " >> $SHPath"/"$count"_"$softName".sh"
			echo "" /usr/local/diamond-0.8.29/diamond blastx --query $fasta --threads 10 --db $DBS/uniref90.fasta --out  diamX_uniref90.outfmt6 --outfmt 6 --max-target-seqs 1  "" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\" /usr/local/diamond-0.8.29/diamond blastx --query $fasta --threads 10 --db $DBS/uniref90.fasta --out  diamX_uniref90.outfmt6 --outfmt 6 --max-target-seqs 1   \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
			
			#3 recherche de dommaines
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			echo "# 3 recherche de dommaines " >> $SHPath"/"$count"_"$softName".sh"
			echo "" hmmscan --cpu 8 --domtblout $resultsDir/$shortName"_PFAM.out" $DBS/Pfam-A.hmm $resultsDir/$shortName.fasta.transdecoder.pep \> $resultsDir/pfam.log "" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\" hmmscan --cpu 8 --domtblout $resultsDir/$shortName"_PFAM.out" $DBS/Pfam-A.hmm $resultsDir/$shortName.fasta.transdecoder.pep  \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
			
			#4 recheche de peptides signaux
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			echo "# 4 recheche de peptides signaux " >> $SHPath"/"$count"_"$softName".sh"
			echo ""/usr/local/signalp-4.1/signalp -f short -n $resultsDir/$shortName"_signalp.out "$resultsDir/$shortName.fasta.transdecoder.pep "" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\"/usr/local/signalp-4.1/signalp -f short -n $resultsDir/$shortName"_signalp.out "$resultsDir/$shortName.fasta.transdecoder.pep   \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
			
			#5 recherche de domaines transmembranaires
			echo " " >> $SHPath"/"$count"_"$softName".sh"
			...
			echo "" /usr/local/Trinotate-3.0.1/util/rnammer_support/RnammerTranscriptome.pl --transcriptome $fasta —org_type (arc|bac|euk) --path_to_rnammer /usr/local/rnammer-1.2/rnammer"" >> $SHPath"/"$count"_"$softName".sh"
			echo "cmd=\" /usr/local/Trinotate-3.0.1/util/rnammer_support/RnammerTranscriptome.pl --transcriptome $fasta —org_type (arc|bac|euk) --path_to_rnammer /usr/local/rnammer-1.2/rnammer   \"" >> $SHPath"/"$count"_"$softName".sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_"$softName".sh"
