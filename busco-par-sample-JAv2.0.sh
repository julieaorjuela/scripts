#!/bin/bash -a
# -*- coding: utf-8 -*-
## @package ./BUSCO-par-sample-JAv2.0.sh
# @author Julie Orjuela,
# adapté pour lancer BUSCO sur la base d'un script de Sebastien Ravel.
# created : 10/2018
# Last: 11/04/2019 Modifié par Valentin Klein.
# Pour lancer sur le cluster IRD

#### TO DO: 
# Attention avec:
# Le nommage des fasta: ex. Canu-75m.fasta Ra-polished.fasta, le nom de l'echantillon est le string avant le "."
# changer la variable BUSCOPathDB
# verifier les parametres de busco -g genome et -sp caenorhabditis par defaut dans ce script.
...
if [ $fastq != "" ] ; then
	...
	printf "\033[32m \n Fasta file were found in directory: \n"$fastqPath
	...
	do
		if [[ "$f" =~ "fasta" ]] || [[ "$f" =~ "fasta.gz"  ]] || [[ "$f" =~ ".fa"  ]] || [[ "$f" =~ "fa.gz"  ]]; then
			...
			echo "module load bioinfo/augustus/3.3.2" >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			...
			echo " " >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			echo "# Copie du fichier fasta vers la partition /scratch du noeud"  >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			echo "scp "$fastqPath"/"$shortName"* \$pathToScratch/" >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			# Copie des fichiers de config d'augustus sur le scratch
			echo "cp -r /usr/local/augustus-3.3.2/config/ \$pathToScratch/augustus_config"  >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			# change la variable AUGUSTUS_CONFIG_PATH
			echo "export AUGUSTUS_CONFIG_PATH=\$(realpath \$pathToScratch/augustus_config)" >> $SHPath"/"$count"_BUSCO-par-sample.sh" 
			...
			echo ""python3 /usr/local/BUSCO-3.0.2/scripts/run_BUSCO.py -i $fasta -o "$shortName"_busco -l $BUSCOPathDB -m genome -c 1 --long -sp caenorhabditis >> $SHPath"/"$count"_BUSCO-par-sample.sh"
       
			#Printing command executed
			echo "cmd=\" python3 /usr/local/BUSCO-3.0.2/scripts/run_BUSCO.py -i $fasta -o "$shortName"_busco -l $BUSCOPathDB -m genome -c 1 --long -sp caenorhabditis \"" >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			echo "echo \"commande executee: \$cmd\"" >> $SHPath"/"$count"_BUSCO-par-sample.sh"			
			
			# Transfert des données du noeud vers master
			echo " " >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			echo "rm -rf \$pathToScratch/augustus_config" >> $SHPath"/"$count"_BUSCO-par-sample.sh"
			...
			# Suppression du repertoire tmp du noeud
			echo " " >> $SHPath"/"$count"_BUSCO-par-sample.sh"	
...
/bin/bash '$pathAnalysis'/sh/${SGE_TASK_ID}_BUSCO-par-sample.sh
	...
	# Print final infos
	printf "\n\n You want run BUSCO-par-sample for "$count" fasta files,
